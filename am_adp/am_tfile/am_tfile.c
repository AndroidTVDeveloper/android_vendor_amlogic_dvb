...
static int timer_get_total(void *timer)
{
	...
	AM_DEBUG(1, "[tfile] total [%d, %d - %d]ms [%lld - %lld]B",
...
static int timer_add_fragment(void *timer, loff_t offset, int force)
{
	...
	AM_DEBUG(1, "[tfile] add fragment %p[time:%d start:%lld]",
...
static loff_t timer_remove_earliest_fragment(void *timer, loff_t *next)
{
	...
	if (!list_empty(&ptimer->list))
	{
		...
		AM_DEBUG(1, "[tfile] del fragment %p[time:%d start:%lld end:%lld size:%lld]",
...
static int aml_tfile_attach_timer(AM_TFile_t tfile)
{
	...
	if (!timer || !frag) {
		AM_DEBUG(1, "[tfile] FATAL: no mem for tfile");
		err = -1;
		goto end;
	}
...
static AM_TFile_Sub_t *aml_timeshift_new_subfile(AM_TFile_t tfile)
{
	...
	if (sub_file == NULL)
	{
		AM_DEBUG(1, "[tfile] Cannot write sub file , no memory!\n");
		return ret;
	}
	...
	{
		AM_DEBUG(1, "[tfile] sub file '%s': %s", fname, strerror(errno));
		sub_file->wfd = -1;
		sub_file->rfd = -1;
	}
	else
	...
	if (sub_file->wfd < 0 || sub_file->rfd < 0)
	{
		...
		else
		{
			...
			{
				AM_DEBUG(1, "[tfile] sub files end at index %d", sub_file->findex-1);
			}
		}
...
static ssize_t aml_timeshift_subfile_read(AM_TFile_t tfile, uint8_t *buf, size_t size)
{
	...
	{
		AM_DEBUG(1, "[tfile] No sub files, cannot read\n");
		return -1;
	}
	...
	if (tfile->cur_rsub_file == NULL)
	{
		...
		AM_DEBUG(1, "[tfile] Reading from the start, file index %d", tfile->cur_rsub_file->findex);
	}
...
static ssize_t aml_timeshift_subfile_write(AM_TFile_t tfile, uint8_t *buf, size_t size)
{
	...
	if (tfile->sub_files == NULL)
	{
		AM_DEBUG(1, "[tfile] No sub files, cannot write\n");
		return -1;
	}
...
AM_ErrorCode_t AM_TFile_Close(AM_TFile_t tfile)
{
	if (! tfile->opened)
	{
		AM_DEBUG(1, "[tfile] has not opened");
		return AM_FAILURE;
	}
...
ssize_t AM_TFile_Write(AM_TFile_t tfile, uint8_t *buf, size_t size)
{
	...
	{
		size = fsize;
		AM_DEBUG(1, "[tfile] data lost, write size(%zu) > file size(%lld)?", size, fsize);
	}
	...
	if (len1 > 0)
	{
		...
		if (ret != (ssize_t)len1) {
			AM_DEBUG(1, "[tfile] data lost, eof write: try:%d act:%d", len1, ret);
			goto write_done;
		}
	...
	if (len2 > 0)
	{
		...
		if (ret2 != (ssize_t)len2) {
			AM_DEBUG(1, "[tfile] data lost, sof write: try:%d act:%d", len2, ret);
			goto write_done;
		}
	...
	if (size > 0)
	{
		...
		if (size > sleft) {
			if (tfile->timer) {
				...
				tfile->total -= remove;
				AM_DEBUG(1, "[tfile] size:%zu w >> s, s:%lld, t:%lld", size, tfile->start, tfile->total);
				debug = 1;
			} else {
		...
		if (size > rleft) {
			...
			//AM_DEBUG(1, "ttt size:%zu w >> r, r:%lld, a:%lld",
		...
		if (debug) {
			AM_DEBUG(1, "[tfile] size:%zu now -> s:%lld, r:%lld, w:%lld, t:%lld, a:%lld",
				size,
...
int AM_TFile_Seek(AM_TFile_t tfile, loff_t offset)
{
	...
	AM_DEBUG(0, "[tfile] Seek: start %lld, read %lld, write %lld, avail %lld, total %lld",
...
int AM_TFile_TimeSeek(AM_TFile_t tfile, int offset_ms/*offset from time start*/)
{
	...
	if (tfile->timer) {
		...
		AM_DEBUG(0, "[tfile] >>timeseek: pstart: %p[time:%d offset:%d - %d]", pstart, pstart->time, pstart->start, pstart->end);
		AM_DEBUG(0, "[tfile] >>timeseek: got block: next%p[time:%d offset:%d - %d]", p, p->time, p->start, p->end);
		AM_DEBUG(0, "[tfile] >>timeseek: start %lld, read %lld, write %lld, avail %lld, total %lld",
				tfile->start, tfile->read, tfile->write, tfile->avail, tfile->total);
		ret = 0;
	}
	pthread_mutex_unlock(&tfile->lock);
...
int AM_TFile_TimeGetReadNow(AM_TFile_t tfile)
{
	...
	if (tfile->timer) {
		...
		AM_DEBUG(0, "[tfile] >>read:\ttime[%d]\toffset[%lld]", now, tfile->read);
	}
	pthread_mutex_unlock(&tfile->lock);
	return now;
}
...
int AM_TFile_TimeGetStart(AM_TFile_t tfile)
{
	...
	if (tfile->timer) {
		...
		tfragment_t *pstart = list_first_entry(get_list_start(timer), tfragment_t, head);
		time = pstart->time;
		AM_DEBUG(1, "[tfile] >>start:\ttime[%d]\toffset[%lld]", pstart->time, pstart->start);
	}
	pthread_mutex_unlock(&tfile->lock);
	return time;
}
...
int AM_TFile_TimeGetEnd(AM_TFile_t tfile)
{
	...
	if (tfile->timer) {
		...
		tfragment_t *pend = list_entry(timer->list.prev, tfragment_t, head);
		time = pend->time;
		AM_DEBUG(1, "[tfile] >>end:\ttime[%d]\toffset[%lld]", pend->time, pend->start);
	}
	pthread_mutex_unlock(&tfile->lock);
	return time;
}
